<template>
  <ModalAction
    id="confirm-delete-users-demo-modal"
  ...
  >
    <div class="text-center">
      ...
      >
        <InputText
          id="confirm-delete-users-demo-name"
          ...
          @update:model-value="confirmationError = ''"
        />
      </div>
...
const expectedName = computed(() => props.userName?.trim() ?? '')

const isNameConfirmed = computed(() => {
  if (!expectedName.value) return true
  return confirmationName.value.trim() === expectedName.value
})

const canConfirm = computed(() =>
  Boolean(props.userId) && isNameConfirmed.value && !isDeleting.value,
)
