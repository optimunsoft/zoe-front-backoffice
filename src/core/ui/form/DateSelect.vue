  ...
  <div class="relative">
    ...
    >
      <div v-show="dropdownOpen" class="z-10 absolute top-full right-0 w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 py-1.5 rounded-lg shadow-lg overflow-hidden mt-1">
        ...
        >
          <button
            v-for="option in DATE_PERIOD_OPTIONS"
            :key="option.id"
            class="flex items-center w-full hover:bg-gray-50 dark:hover:bg-gray-700/20 py-1 px-3 cursor-pointer"
            :class="option.id === model && 'text-violet-500'"
            @click="selectOption(option.id)"
          >
            <svg class="shrink-0 mr-2 fill-current text-violet-500" :class="option.id !== model && 'invisible'" width="12" height="9" viewBox="0 0 12 9">
...
const model = defineModel<DatePeriodId>({ default: DATE_PERIOD_DEFAULT })
...
const selectedLabel = computed(
  () => DATE_PERIOD_OPTIONS.find((option) => option.id === model.value)?.period ?? '',
)

const selectOption = (id: DatePeriodId) => {
  model.value = id
